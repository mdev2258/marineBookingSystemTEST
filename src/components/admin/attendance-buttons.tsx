...
type Marked = 'paid' | 'completed' | 'no_show';

/**
 * The yard control. One thumb, bright sun, possibly wet hands:
 * two 48px targets, colour flips on touch, no dropdown and no modal.
 *
 * useOptimistic rather than a spinner, because down by the crane with one bar
 * of signal the round trip is long enough to make the operator tap twice.
 */
export function AttendanceButtons({
  ...
  function choose(next: 'completed' | 'no_show') {
  ...
  return (
    <div className="mt-3 flex gap-2">
      <button
        ...
        aria-label={`Mark work on ${vesselName} done`}
      ...
      <button
        ...
        aria-label={`Mark ${vesselName} not ready`}
